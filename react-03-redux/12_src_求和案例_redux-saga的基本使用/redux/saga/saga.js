import {take, fork, cancel, delay, call, put, takeEvery, takeLatest, takeLeading, takeMaybe} from "redux-saga/effects";
...
function* watchSaga() {
     ...
     * 等效于 take+fork
     */
    // yield takeEvery(ASYNC_INCREMENT, asyncAddSaga);

    while (true) {
        // take监听组件发来的action
        const action = yield take(ASYNC_INCREMENT);
        ...
        // fork同步执行异步处理函数
        task = yield fork(asyncAddSaga, action);
    }
}

function* asyncAddSaga(action) {
    ...
    yield delay(time);
    // call函数发异步请求，阻塞执行
    let res = yield call(() => asyncAdd(data)); // asyncAdd是一个返回Promise对象的函数
